...
(async () => {
  ...
  try {
    ...
    await app.evaluate(({ app }) => {
      ...
      const imageOptions = [1, 2, 3, 4].map(i => ({ origin:i===1?'listing':'detail', source: `https://example.com/image${i}.jpg`, preview }));
    ...
    const open = async (manualFirst = false, imageCount = 4) => {
    ...
    checks.push("four source-labelled preview choices default first; choosing second preserves existing photo until save and passes pending download");
    dialog = await open();
    ...
    await expect(dialog.getByRole("button", { name: /^选择网站图片 / })).toHaveCount(4);
    ...
    await batch.getByRole("button", { name: "选择网站图片 4", exact: true }).click();
    ...
    assert.equal(patches[0].imageSource, "https://example.com/image4.jpg");
    ...
    checks.push("batch candidate editor selects fourth thumbnail or skips without writing equipment");
